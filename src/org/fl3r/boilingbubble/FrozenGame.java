...
public class FrozenGame extends GameScreen {
	...
	public FrozenGame(BmpWrap background_arg, BmpWrap[] bubbles_arg,
			...
			SoundManager soundManager_arg, LevelManager levelManager_arg) {
		...
		for (int j = 0; j < 12; j++) {
			for (int i = j % 2; i < 8; i++) {
				if (currentLevel[i][j] != -1) {
					BubbleSprite newOne = new BubbleSprite(new Rect(190 + i
							* 32 - (j % 2) * 16, 44 + j * 28, 32, 32),
	...
	private Sprite restoreSprite(Bundle map, Vector imageList, int i) {
		...
		if (type == Sprite.TYPE_BUBBLE) {
			...
			return new BubbleSprite(new Rect(left, top, right, bottom), color,
					moveX, moveY, realX, realY, fixed, blink, released,
	...
	public void restoreState(Bundle map, Vector imageList) {
		...
		for (int i = 0; i < 8; i++) {
			for (int j = 0; j < 13; j++) {
				...
				if (spriteIdx != -1) {
					bubblePlay[i][j] = (BubbleSprite) savedSprites
							.elementAt(spriteIdx);
				} else {
					bubblePlay[i][j] = null;
				}
		...
		if (movingBubbleId == -1) {
			...
			movingBubble = (BubbleSprite) savedSprites
	...
	public boolean play(boolean key_left, boolean key_right, boolean key_fire,
			double trackball_dx, boolean touch_fire, double touch_x,
			double touch_y, boolean ats_touch_fire, double ats_touch_dx) {
		...
		if (!ats && touch_fire && movingBubble == null) {
			...
			launchBubblePosition = (Math.PI - Math.atan2(yy, xx)) * 40.0
					/ Math.PI;
			...
			if (move[FIRE] == KEY_UP || hurryTime > 480) {
				if (movingBubble == null && readyToFire) {
					...
					movingBubble = new BubbleSprite(new Rect(302, 390, 32, 32),
							(int) launchBubblePosition, currentColor,
		...
		if (movingBubble != null) {
			...
			if (movingBubble.fixed()) {
				if (movingBubble.getSpritePosition().y >= 380
					...
					this.addSprite(new ImageSprite(
							new Rect(152, 190, 337, 116), gameWon));
					// TODO
